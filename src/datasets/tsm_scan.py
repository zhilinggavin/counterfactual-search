...
class CTScan(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        try:
            ...
            self.scan_casename = self.scan_path.parts[-2]
        except:
            # self.scan = np.asarray(imageio.imread(scan_path)).astype(np.int16)
            # self.scan = self.scan[:,:,0]
            # self.segm = np.asarray(imageio.imread(labels_path)).astype(np.uint16)
            print('retrying')
            sleep(5)
            self.scan = self.load_volume(scan_path, np.int16)
            self.segm = self.load_volume(labels_path, np.uint8)
            print(f'Unable to load {labels_path}')
    ...
    def __getitem__(self, index):
        ...
        try:
            scan_slice = self.get_ith_slice(self.scan, slice_index)  # range in HU
        ...
        if self.load_masks:
            ...
            masks = [
                (label_slice == self.class_to_idx[c]).astype(np.uint8) if c != 'zero_mask' else np.zeros_like(label_slice, np.uint8)
                for c in classes
                if c != 'gaussian'
            ]

        # normalize image
        scan_slice = self.norm(scan_slice, label_slice)  # CTnorm[-1, 1], MinMAXnorm[0,1]
        ...
        if self.synth_params:
            ...
            if self.labels is None and self.default_label is not None:
                ...
                assert (
                    self.classify_labels is not None
                ), 'Unable to determine the anomaly label for the slice. Set synthetic augmentation of classify_labels parameters'
        ...
        sample['scan_casename'] = self.scan_casename
...
class CTSlice(torch.utils.data.Dataset):
    slicing_dims = {
        'sagittal': 0,  # side view
        'coronal': 1,  # front view
        'axial': 2,  # top down view
    ...
    def __getitem__(self, index):
        ...
        self.scan = np.asarray(imageio.imread(self.scan_path)).astype(np.int16)
        self.scan = self.scan[:, :, 0]
        self.segm = np.asarray(imageio.imread(self.scan_path)).astype(np.uint16)
        assert self.scan.shape == self.segm.shape, 'Shapes of provided scan and labels volumes do not match'
