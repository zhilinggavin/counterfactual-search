...
def get_transforms(opt):
    ...
    if 'fib' in opt.datasets[0]['kind']:
        ...
        print('Using CT norm to change img from HU to [-1,1], no normalization applied in transforms(Aug)')
...
def nnunet_transforms(opt):
    ...
    rotation_for_DA = {'x': (-np.pi, np.pi), 'y': 0, 'z': 0}
    ...
    tr_transforms.append(
        GaussianBlurTransform((0.5, 1.0), different_sigma_per_channel=True, p_per_sample=0.2, p_per_channel=0.5, data_key=data_key, seg_key=seg_key)
    )
