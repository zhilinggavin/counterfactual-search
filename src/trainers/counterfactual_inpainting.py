...
class CounterfactualInpaintingTrainer(CounterfactualTrainer):
    ...
    def evaluate_counterfactual(self, loader, phase='val', tau=0.8, skip_fid=False, postprocess_morph: bool = False):
        ...
        each_dice = []
        train_eval_epoch = self.opt.get('train_eval_epoch') if phase == 'train' else len(loader)
        ...
        for i, batch in tqdm(enumerate(loader), desc='Validating counterfactuals', leave=False, total=train_eval_epoch or len(loader), ncols=120):
            ...
            try:
                ...
                cf_gt_masks = batch['mask'][:, self.cf_gt_seg_mask_idx].cuda(non_blocking=True)
            cf_gt_masks = cf_gt_masks.unsqueeze(1)  # B x 1 x H x W
            labels = batch['label']
            ...
            if abnormal_mask.any():
                ...
                try:
                    self.val_iou_xc.update(diff_seg[abnormal_mask], cf_gt_masks[abnormal_mask])
                    self.val_dice_xc.update(diff_seg[abnormal_mask], cf_gt_masks[abnormal_mask])
                except:
                    self.val_iou_xc.update(diff_seg[abnormal_mask].squeeze(1), cf_gt_masks[abnormal_mask])
                    self.val_dice_xc.update(diff_seg[abnormal_mask].squeeze(1), cf_gt_masks[abnormal_mask])

            if i <= 50:
                vis_confmat = confmat_vis_img(cf_gt_masks[0].unsqueeze(0), diff_seg[0].unsqueeze(0), normalized=True)[0]
                vis = torch.stack(
                    (
                        real_imgs[0],
                ...
                vis_path = cf_dir / (
                    f'epoch_%d_counterfactual_%d_label_%d_true_%d_pred_%d.png'
                    % (self.current_epoch, i, labels[0], real_f_x_desired_discrete[0][0], gen_f_x_discrete[0][0])
                )
                save_image(vis.data, vis_path, nrow=3, normalize=False)  # value_range=(0, 1)
        ...
        self.logger.info(f'CV(X, Xc) = {cv_score:.3f} (τ={tau}, posterior_num_samples={pos_true_mask.sum()})')

        # IoU
        cf_iou_xc = self.val_iou_xc.compute().item()
        self.val_iou_xc.reset()
        # Dice
        cf_dice_xc = self.val_dice_xc.compute().item()
        self.val_dice_xc.reset()

        self.logger.info(
            f'IoU(S, Sc) = {cf_iou_xc:.3f}; Dice = {cf_dice_xc:.3f}; (cf_thresh={self.cf_threshold}, pred_abnormal_num_samples={pred_num_abnormal_samples}, mask_id={self.cf_gt_seg_mask_idx})'
        )
        ...
        self.logger.info(f'Ratio of predicted abnormal slices to true abnormal slices: {pred_num_abnormal_samples / (max(true_num_abnormal_samples, 1e-8))}')
        return {
            'counter_acc': cacc,
    ...
    @torch.no_grad()
    def test_counterfactual(self, loader, phase='val', tau=0.8, skip_fid=False, postprocess_morph: bool = False):
        self.model.eval()
        ...
        for i, batch in tqdm(enumerate(loader), desc='Validating counterfactuals', leave=False, total=train_eval_epoch or len(loader), ncols=120):
            ...
            if abnormal_mask.any():
                ...
                each_dice.append(self.val_dice_xc.compute().item())
                seg = diff_seg.squeeze()
                seg = seg.flip((0, 1)).transpose(1, 0).cpu().numpy()
                seg = (seg * 255).astype(np.uint8)

                tmp_dir = '/media/NAS06/gavinyue/disentanglement/benchmark/counterfactual-search/seg_results'
                Image.fromarray(seg).save(f'{tmp_dir}/orig{i:05d}.png')

                # save compare visualization
                vis_confmat = confmat_vis_img(cf_gt_masks[0].unsqueeze(0), diff_seg[0].unsqueeze(0), normalized=True)[0]
                vis_confmat = vis_confmat.flip((0, 1)).transpose(1, 0)
                ...
                for idx, img in enumerate(vis):
                    vis[idx] = img.flip((0, 1)).transpose(1, 0)
                ...
                # save first example for visualization
                vis_path = f'{tmp_dir}/compare/orig{i:05d}_compare.png'
                save_image(vis.data, vis_path, nrow=3, normalize=False)  # value_range=(0, 1)
    ...
    def test_counterfactual_genlabel1(self, loader, phase='val', tau=0.8, skip_fid=False, postprocess_morph: bool = False):
        ...
        cf_dir = self.cf_vis_dir_train if phase == 'train' else self.cf_vis_dir_val
        cf_dir = cf_dir.with_name('val_genlabel1')
        ...
        for i, batch in tqdm(enumerate(loader), desc='Validating counterfactuals', leave=False, total=train_eval_epoch or len(loader), ncols=120):
            ...
            if normal_mask.any():
                if postprocess_morph:
                    diff_seg[normal_mask] = self.postprocess_morph(diff_seg[normal_mask])
                pred_num_abnormal_samples += normal_mask.sum()
                self.val_iou_xc.update(diff_seg[normal_mask].squeeze(1), cf_gt_masks[normal_mask])
                self.val_dice_xc.update(diff_seg[normal_mask].squeeze(1), cf_gt_masks[normal_mask])
                ...
                Image.fromarray(seg).save(f'{cf_dir}/orig{i:05d}_cf_origlabel{labels[0][0]}_pred{gen_f_x_discrete[0][0]}.png')
                ...
                # save first example for visualization
                (cf_dir / 'vis_comparison').mkdir(parents=True, exist_ok=True)
                
                vis_path = f'{cf_dir}/vis_comparison/orig{i:05d}_label{labels[0][0]}_desired{real_f_x_desired_discrete[0][0]}_pred{gen_f_x_discrete[0][0]}.png'
                save_image(vis.data, vis_path, nrow=3, normalize=False)  # value_range=(0, 1)
...
class CounterfactualInpaintingV2Trainer(CounterfactualInpaintingTrainer):
    ...
    def evaluate_counterfactual(self, loader, phase='val', tau=0.8, skip_fid=False, postprocess_morph: bool = False):
        ...
        pred_num_abnormal_samples = 0
        true_num_abnormal_samples = 0
        for i, batch in tqdm(enumerate(loader), desc='Validating counterfactuals', leave=False, total=len(loader), ncols=120):
            # Evaluate Counterfactual Validity Metric
            real_imgs = batch['image'].cuda(non_blocking=True)
            # cf_gt_masks = batch['masks'][:, self.cf_gt_seg_mask_idx].cuda(non_blocking=True)
            cf_gt_masks = batch['mask'][:, self.cf_gt_seg_mask_idx].cuda(non_blocking=True)
