...
from src.attributors import ATTRIBUTORS, get_attributor
...
parser.add_argument(
    '-cft', '--cf_threshold', type=float, required=False, default=0.25, help='Theshold to be used for thresholding counterfactual maps into segmentation masks'
)
parser.add_argument('-pcf', '--postprocess_cf', action='store_true', default=False, help='Whether to postprocess CF masks generated by the model')
parser.add_argument(
    '-st', '--sal_threshold', type=float, required=False, default=0.975, help='Theshold to be used for thresholding saliency maps into segmentation masks'
)
...
arg = [
    '-cp',
    '/media/NAS06/gavinyue/disentanglement/benchmark/counterfactual-search/tools/training_logs/counterfactual/fibrosis/inpainting_counterfactual_cgan-August-28-2024_11+46PM-fibrosis+covid/checkpoints/checkpoint_80.pth',
]
opt = parser.parse_args(arg)
# opt.cp = "/media/NAS06/gavinyue/disentanglement/benchmark/counterfactual-search/tools/training_logs/counterfactual/fibrosis/inpainting_counterfactual_cgan-August-28-2024_11+46PM-fibrosis+covid/checkpoints/checkpoint_80.pth"
...
def eval_attributors(attrib_kinds: list[str], cf_trainer: CounterfactualTrainer, loader: torch.utils.data.DataLoader, sal_threshold: float = 0.95):
    ...
    for i, batch in tqdm(enumerate(loader), desc=f'Validating {list(attrib_kinds)}', leave=False, total=len(loader)):
        ...
        for k, attrib in attribs.items():
            ...
            vis = torch.stack(
                (
                    vis_img,
                    (vis_img * 0.3 + sal_maps[0] * 0.7),
                    torch.zeros_like(vis_img),
                    torch.zeros_like(vis_img),
                ),
                dim=0,
            ).permute(0, 2, 3, 1)
    ...
    for k, iou in ious.items():
        ...
        cf_trainer.logger.info(
            f'[{k}] IoU(S, Sc) = {sal_iou:.3f} (sal_thresh={sal_threshold}, num_samples={num_abnormal_samples}, mask={cf_trainer.cf_gt_seg_mask_idx})'
        )
